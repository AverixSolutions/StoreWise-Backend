// backend/src/routes/sync/purchaseReturn.routes.ts
...
// POST /api/sync/purchaseReturn/push
router.post("/push", async (req: Request, res: Response) => {
  ...
  for (const r of records) {
    ...
    try {
      ...
      console.error("[purchaseReturn push] failed for", r.id, err.message);
...
// GET /api/sync/purchaseReturn/pull
...
// POST /api/sync/purchaseReturnItem/push
router.post("/items/push", async (req: Request, res: Response) => {
  ...
  for (const r of records) {
    try {
      ...
      const parent = await prisma.purchaseReturn.findFirst({
      ...
      console.error("[purchaseReturnItem push] failed for", r.id, err.message);
      results.push({ id: r.id, accepted: false, reason: err.message });
    }
  }
  res.json({ results, pushedAt: now.toISOString() });
...
// GET /api/sync/purchaseReturn/items/pull
router.get("/items/pull", async (req: Request, res: Response) => {
  const licenseId = getLicenseId(req);
  const since = req.query.since as string | undefined;
  const PAGE_SIZE = 500;
  try {
    const where: any = { purchaseReturn: { licenseId } };
    if (since) where.updatedAt = { gt: new Date(since) };
